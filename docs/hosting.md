...
> [!hint]
> Some Quartz features (like [RSS Feed](features/RSS%20Feed.md) and sitemap generation) require `baseUrl` to be configured properly in your [configuration](configuration.md) to work properly. Make sure you set this before deploying!
...
![dns records](images/dns%20records.png)_The above shows a screenshot of Google Domains configured for both `jzhao.xyz` (an apex domain) and `quartz.jzhao.xyz` (a subdomain)._
